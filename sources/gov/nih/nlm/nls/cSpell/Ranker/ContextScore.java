...
public class ContextScore
{
    ...
    *
    * @param     inTerm target token or candidate (can be multiword)
    * @param     contextVec wordVec of context from IM
    * @param     word2Vec word2Vec matrix of OM
    */
    ...
    {
        String outStr = term_ + fieldSepStr 
            + String.format("%1.8f", score_);    //match up to the PRECISION
        return outStr;    
    }
    // private method
    // Test merge and Split
    private static void Test(String inText, int tarPos, 
        int tarSize, int radius, String mergedWord, String splitWords,
        Word2Vec w2vIm, Word2Vec w2vOm)
    {
        ...
        DoubleVec contextVecA = Word2VecContext.GetContextVec(tarPos, tarSize,
        ...
        System.out.println(score1.ToString() + "|" 
            + String.format("%1.8f", score1a.GetScore()));
        // 2. split words
        ...
        System.out.println(score2.ToString() + "|" 
            + String.format("%1.8f", score2a.GetScore()));
        // 3. 3. 3. Use avg. score on single words
        // This method use different context for each single word
        ArrayList<String> splitWordList = TermUtil.ToWordList(splitWords);
        int index = 0;
        ...
        {
            // window radius
            DoubleVec contextVecS = Word2VecContext.GetContextVec(
                tarPos+index, 1, nonSpaceTokenList, w2vIm, radius, 
                word2VecSkipWord, debugFlag);
            ContextScore scoreS 
                = new ContextScore(splitWord, contextVecS, w2vOm);    
            //System.out.println("-- " + scoreS.ToString());    
            scoreSAvg += scoreS.GetScore();
            // all text
            DoubleVec contextVecSA = Word2VecContext.GetContextVec(
                tarPos+index, 1, nonSpaceTokenList, w2vIm, word2VecSkipWord, 
                debugFlag);
            ContextScore scoreSA 
                = new ContextScore(splitWord, contextVecSA, w2vOm);    
        ...
        System.out.println("Avg. Single Word|" 
            + String.format("%1.8f", scoreSAvg) + "|"
            + String.format("%1.8f", scoreSAAvg));
    }
    private static void Tests(Word2Vec w2vIm, Word2Vec w2vOm)
    {
        // test 1: non-word split
        System.out.println("======== non-word split ==========");
        ...
        System.out.println("======== non-word merge ==========");
        ...
        System.out.println("======== real-word split ==========");
        // 1034, along -> a long
        ...
        System.out.println("======== real-word merge ==========");
        // 1, on set -> onset
    ...
    private static void TestOnSet(Word2Vec w2vIm, Word2Vec w2vOm)
    {
        ...
        System.out.println("- [" + str2 + "]: " + s2.ToString());
    }
    // test Driver
    public static void main(String[] args)
    {
        if(args.length > 0)
        {
            System.out.println("Usage: java ContextScore");
            System.exit(0);
        }
        // TBD: test
    ...
    private String term_ = new String();
